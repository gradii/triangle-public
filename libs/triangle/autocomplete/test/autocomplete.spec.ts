...
import {
  AutocompleteOrigin,
  AutocompleteTrigger,
  TRI_AUTOCOMPLETE_DEFAULT_OPTIONS,
  TriAutocomplete,
  TriAutocompleteModule,
} from "@gradii/triangle/autocomplete";
import {
  OptionComponent,
...
import {
  getTriAutocompleteMissingPanelError,
  TRI_AUTOCOMPLETE_SCROLL_STRATEGY,
} from "../src/common";
...
describe("autocomplete", () => {
  ...
  describe("panel toggling", () => {
    ...
    it("should hide the panel when the options list is empty", fakeAsync(() => {
      ...
      const panel = overlayContainerElement.querySelector(
        ".tri-autocomplete-panel"
      ) as HTMLElement;

      expect(panel.classList).toContain(
        "tri-autocomplete-visible",
    ...
    it("should toggle the visibility when typing and closing the panel", fakeAsync(() => {
      ...
      expect(
        overlayContainerElement.querySelector(".tri-autocomplete-panel")!
          .classList
      ).toContain("tri-autocomplete-visible", "Expected panel to be visible.");
      ...
      expect(
        overlayContainerElement.querySelector(".tri-autocomplete-panel")!
          .classList
      ).toContain("tri-autocomplete-hidden", "Expected panel to be hidden.");
    ...
    const rtlFixture = createComponent(SimpleAutocomplete, [
      {
        ...
        useFactory: () => ({ value: "rtl", change: EMPTY }),
      },
    ]);
  ...
  describe("forms integration", () => {
    ...
    it("should disable the input when used with a value accessor and without `triInput`", () => {
      overlayContainer.ngOnDestroy();
      fixture.destroy();
  ...
  describe("keyboard events", () => {
    ...
    it("should set the active item to the first option when DOWN key is pressed", () => {
      ...
      expect(optionEls[0].classList).toContain("tri-option-active");
      expect(optionEls[1].classList).not.toContain("tri-option-active");
      ...
      expect(optionEls[0].classList).not.toContain("tri-option-active");
      expect(optionEls[1].classList).toContain("tri-option-active");
    });
    ...
    it("should set the active item to the last option when UP key is pressed", () => {
      ...
      expect(optionEls[10].classList).toContain("tri-option-active");
      expect(optionEls[0].classList).not.toContain("tri-option-active");
    ...
    it("should scroll to active options below the fold", () => {
      ...
      const scrollContainer = document.querySelector(
        ".cdk-overlay-pane .tri-autocomplete-panel"
      )!;
    ...
    it("should scroll to active options on UP arrow", () => {
      ...
      // Expect option bottom minus the panel height (32*11 - 256 = 96)
      expect(scrollContainer.scrollTop).toEqual(
        96,
    ...
    it("should not scroll to active options that are fully in the panel", () => {
      ...
      // Expect option bottom minus the panel height (32*9 - 256 = 32)
      expect(scrollContainer.scrollTop).toEqual(
        32,
        `Expected panel to reveal the sixth option.`
      );
  ...
  describe("option groups", () => {
    ...
    it("should scroll to active options that are above the panel", fakeAsync(() => {
      ...
      // Expect to show the top of the 2nd option at the top of the panel.
      // It is offset by 32, because there's a group label above it.
      expect(container.scrollTop).toBe(
        32 * 2,
        "Expected panel to scroll up when option is above panel."
      );
  ...
  describe("Fallback positions", () => {
    it("should use below positioning by default", fakeAsync(() => {
      ...
      expect(Math.floor(inputBottom) + AutocompleteMarginHeight).toEqual(
        Math.floor(panelTop),
        `Expected panel top to match input bottom by default.`
      );
      expect(panel.classList).not.toContain("tri-autocomplete-panel-above");
    }));
    ...
    it("should fall back to above position if panel cannot fit below", fakeAsync(() => {
      ...
      expect(panel.classList).toContain("tri-autocomplete-panel-above");
    }));
    ...
    it('should align panel properly when filtering in "above" position', fakeAsync(() => {
      ...
      expect(Math.floor(inputTop)).toEqual(
        Math.floor(panelBottom) + AutocompleteMarginHeight,
    ...
    it("should fall back to above position when requested if options are added while the panel is open", fakeAsync(() => {
      ...
      expect(Math.floor(panelRect.top)).toBe(
        Math.floor(inputRect.bottom) + AutocompleteMarginHeight,
      ...
      expect(Math.floor(panelRect.bottom) + AutocompleteMarginHeight).toBe(
  ...
  describe("Option selection", () => {
    ...
    it("should be able to preselect the first option", fakeAsync(() => {
      ...
      expect(
        overlayContainerElement.querySelectorAll("tri-option")[0].classList
      ).toContain(
        "tri-option-active",
    ...
    it("should be able to configure preselecting the first option globally", fakeAsync(() => {
      ...
      fixture = createComponent(SimpleAutocomplete, [
        {
          provide: TRI_AUTOCOMPLETE_DEFAULT_OPTIONS,
    ...
    it("should handle `optionSelections` being accessed too early", fakeAsync(() => {
      ...
      expect(spy).toHaveBeenCalledWith(jasmine.any(TriOptionSelectionEvent));
      subscription!.unsubscribe();
    }));

    // it('should reposition the panel when the amount of options changes', fakeAsync(() => {
    //   let inputReference = .querySelector('.tri-form-field-flex');
  ...
  describe("panel closing", () => {
    ...
    it("should emit panel close event when selecting an option", () => {
      ...
      expect(closingActionSpy).toHaveBeenCalledWith(
        jasmine.any(TriOptionSelectionEvent)
      );
    });
  ...
  describe("misc", () => {
    ...
    it("should filter properly with ngIf after setting the active item", () => {
      ...
      expect(fixture.componentInstance.OptionComponents.length).toBe(2);
    });
    ...
    it("should throw if the user attempts to open the panel too early", () => {
      ...
      expect(() => {
        fixture.componentInstance.trigger.openPanel();
      }).toThrow(getTriAutocompleteMissingPanelError());
    });
    ...
    it("should transfer the tri-autocomplete classes to the panel element", fakeAsync(() => {
      const fixture = createComponent(SimpleAutocomplete);
      fixture.detectChanges();
      ...
      const autocomplete =
        fixture.debugElement.nativeElement.querySelector("tri-autocomplete");
      const panel = overlayContainerElement.querySelector(
        ".tri-autocomplete-panel"
      )!;
    ...
    it("should remove old classes when the panel class changes", fakeAsync(() => {
      ...
      expect(classList).toContain("tri-autocomplete-visible");
  ...
  it("should emit an event when an option is selected", fakeAsync(() => {
    ...
    const event = spy.calls.mostRecent()
      .args[0] as TriAutocompleteSelectedEvent;
...
@Component({
  template: `
    <div class="tri-form-field-flex" [style.width.px]="width">
      <input
        triInput
        placeholder="State"
        [triAutocomplete]="auto"
        [triAutocompleteDisabled]="autocompleteDisabled"
    ...
    <tri-autocomplete
      [class]="panelClass"
      #auto="triAutocomplete"
      [displayWith]="displayFn"
      (opened)="openedSpy()"
      (closed)="closedSpy()"
    >
      <tri-option *ngFor="let state of filteredStates" [value]="state">
...
class SimpleAutocomplete implements OnDestroy {
  ...
  @ViewChild(TriAutocomplete, { static: false }) panel: TriAutocomplete;
...
@Component({
  template: `
    ...
    />
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option
...
@Component({
  template: `
    ...
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let number of numbers" [value]="number">
        <span>{{ number }}</span>
      </tri-option>
    </tri-autocomplete>
  `,
...
@Component({
  ...
  template: `
    <input type="text" triInput [triAutocomplete]="auto" />

    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let option of options" [value]="option">{{
        option
      }}</tri-option>
    </tri-autocomplete>
  `,
...
@Component({
  template: `
    ...
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option-group *ngFor="let group of stateGroups" [label]="group.title">
        <tri-option *ngFor="let state of group.states" [value]="state">
          <span>{{ state }}</span>
        </tri-option>
      </tri-option-group>
    </tri-autocomplete>
...
@Component({
  template: `
    ...
    >
      <tri-option *ngFor="let state of states" [value]="state">
        <span>{{ state }}</span>
      </tri-option>
    </tri-autocomplete>
  `,
...
class AutocompleteWithSelectEvent {
  ...
  @ViewChild(AutocompleteTrigger, { static: false })
  trigger: AutocompleteTrigger;
  @ViewChild(TriAutocomplete, { static: false }) autocomplete: TriAutocomplete;
}

@Component({
  template: `
    <input [formControl]="formControl" [triAutocomplete]="auto" />
    <tri-autocomplete #auto="triAutocomplete"></tri-autocomplete>
  `,
})
...
@Component({
  template: `
    ...
    <tri-autocomplete #auto="triAutocomplete">
      <tri-option *ngFor="let value of values" [value]="value">{{
        value
      }}</tri-option>
    </tri-autocomplete>
  `,
  ...
  template: `
    <div>
      <input
        triInput
        [triAutocomplete]="auto"
        [triAutocompleteConnectedTo]="connectedTo"
        [(ngModel)]="selectedValue"
      />
    </div>
    ...
    <div
      class="origin"
      triAutocompleteOrigin
      #origin="triAutocompleteOrigin"
...
class AutocompleteWithDifferentOrigin {
  @ViewChild(AutocompleteTrigger, { static: false })
  trigger: AutocompleteTrigger;
  @ViewChild(AutocompleteOrigin, { static: false })
  alternateOrigin: AutocompleteOrigin;
  selectedValue: string;
  values = ["one", "two", "three"];
  connectedTo?: AutocompleteOrigin;
}
...
@Component({
  template: '<input [triAutocomplete]="null" triAutocompleteDisabled>',
})
