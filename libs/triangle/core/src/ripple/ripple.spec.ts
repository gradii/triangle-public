...
describe('TriRipple', () => {
  ...
  describe('basic ripple', () => {
    ...
    it('creates ripple on mousedown', () => {
      ...
      expect(rippleTarget.querySelectorAll('.tri-ripple-element').length).toBe(2);
    });
    ...
    it('should launch multiple ripples for multi-touch', fakeAsync(() => {
      ...
      expect(rippleTarget.querySelectorAll('.tri-ripple-element').length).toBe(3);
    ...
    it('should ignore fake mouse events from screen readers', fakeAsync(() => {
      ...
      tick(enterDuration);
      expect(rippleTarget.querySelector('.tri-ripple-element')).toBeFalsy();
    }));
  ...
  describe('manual ripples', () => {
    ...
    it('should remove ripples that are not done fading-in', fakeAsync(() => {
      ...
      expect(rippleTarget.querySelectorAll('.tri-ripple-element').length)
  ...
  describe('with disabled animations', () => {
    ...
    beforeEach(() => {
      TestBed.resetTestingModule();
      TestBed.configureTestingModule({
        imports     : [NoopAnimationsModule, TriRippleModule],
...
@Component({
  template: `
    <div id="container" #ripple="triRipple" triRipple
         style="position: relative; width:300px; height:200px;">
    </div>
  `,
})
class BasicRippleContainer {
  @ViewChild('ripple') ripple: TriRipple;
}
...
@Component({
  template: `
    <div id="container" #ripple="triRipple" triRipple></div>`,
})
