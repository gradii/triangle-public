...
import {
  createMouseEvent, createTouchEvent, dispatchEvent, dispatchFakeEvent, dispatchMouseEvent,
  dispatchTouchEvent,
} from '@angular/cdk/testing/private';
import { DOCUMENT } from '@angular/common';
import {
  AfterViewInit, ChangeDetectionStrategy, Component, ElementRef, Input, NgZone, Provider, QueryList,
  Type, ViewChild, ViewChildren, ViewEncapsulation,
} from '@angular/core';
...
describe('CdkDrag', () => {
  ...
  describe('standalone draggable', () => {
    ...
    it('should add a class while an element is being dragged', fakeAsync(() => {
      ...
      expect(element.classList).not.toContain('tri-drag-dragging');

      startDraggingViaMouse(fixture, element);

      expect(element.classList).toContain('tri-drag-dragging');
    ...
    it('should be able to set the triDrag element as handle if it has a different root element',
      fakeAsync(() => {
    ...
    it('should not be able to drag the element if dragging is disabled', fakeAsync(() => {
      ...
      expect(dragElement.classList).not.toContain('tri-drag-disabled');

      fixture.componentInstance.dragInstance.disabled = true;
      fixture.detectChanges();

      expect(dragElement.classList).toContain('tri-drag-disabled');
      expect(dragElement.style.transform).toBeFalsy();
      dragElementViaMouse(fixture, dragElement, 50, 100);
      expect(dragElement.style.transform).toBeFalsy();
    }));
    ...
    it('should throw if drag item is attached to an ng-container', fakeAsync(() => {
      expect(() => {
        ...
        flush();
      }).toThrowError(/^triDrag must be attached to an element node/);
    }));
  ...
  describe('draggable with a handle', () => {
    ...
    it('should be able to use more than one handle to drag the element', fakeAsync(() => {
      ...
      const dragElement = fixture.componentInstance.dragElement.nativeElement;
      const handles     = fixture.componentInstance.handles.map(
        handle => handle.element.nativeElement);
    ...
    it('should throw if drag handle is attached to an ng-container', fakeAsync(() => {
      expect(() => {
        ...
        flush();
      }).toThrowError(/^triDragHandle must be attached to an element node/);
    }));
  ...
  describe('in a drop container', () => {
    ...
    it('should toggle a class when the user starts dragging an item', fakeAsync(() => {
      ...
      expect(dropZone.element.nativeElement.classList).not.toContain('tri-drop-list-dragging');

      startDraggingViaMouse(fixture, item);

      expect(dropZone.element.nativeElement.classList).toContain('tri-drop-list-dragging');
      ...
      expect(dropZone.element.nativeElement.classList).not.toContain('tri-drop-dragging');
    }));
    ...
    it('should toggle the drop dragging classes if there is nothing to trigger change detection',
      fakeAsync(() => {
        ...
        expect(dropZone.element.nativeElement.classList).not.toContain('tri-drop-list-dragging');
        expect(item.classList).not.toContain('tri-drag-dragging');

        startDraggingViaMouse(fixture, item);

        expect(dropZone.element.nativeElement.classList).toContain('tri-drop-list-dragging');
        expect(item.classList).toContain('tri-drag-dragging');
    ...
    it('should dispatch the `sorted` event as an item is being sorted', fakeAsync(() => {
      ...
      const items       = fixture.componentInstance.dragItems.map(
    ...
    it('should not move the original element from its initial DOM position', fakeAsync(() => {
      ...
      let dragElements = Array.from(root.querySelectorAll('.tri-drag'));
      ...
      dragElements = Array.from(root.querySelectorAll('.tri-drag'));
      expect(dragElements.map(el => el.textContent)).toEqual(['Zero', 'One', 'Two', 'Three']);
    }));
    ...
    it('should create a preview element while the item is dragged', fakeAsync(() => {
      ...
      const preview     = document.querySelector('.tri-drag-preview')! as HTMLElement;
    ...
    it('should be able to constrain the preview position', fakeAsync(() => {
      const fixture                              = createComponent(DraggableInDropZone);
      fixture.componentInstance.boundarySelector = '.tri-drop-list';
      fixture.detectChanges();
      const item     = fixture.componentInstance.dragItems.toArray()[1].element.nativeElement;
      const listRect =
    ...
    it('should not throw when cloning an invalid canvas', fakeAsync(() => {
      ...
      expect(document.querySelector('.tri-drag-preview canvas')).toBeTruthy();
    }));
    ...
    it('should clone the content of descendant input elements', fakeAsync(() => {
      ...
      const preview         = document.querySelector('.tri-drag-preview')!;
    ...
    it('should preserve checked state for radio inputs in the content', fakeAsync(() => {
      ...
      const placeholder           = document.querySelector('.tri-drag-placeholder')!;
    ...
    it('should not create a preview if the element was not dragged far enough', fakeAsync(() => {
      ...
      expect(document.querySelector('.tri-drag-preview')).toBeFalsy();
    }));
    ...
    it('should pass the proper direction to the preview in rtl', fakeAsync(() => {
      ...
      expect(document.querySelector('.tri-drag-preview')!.getAttribute('dir'))
    ...
    it('should reset immediately when failed drag happens after a successful one', fakeAsync(() => {
      ...
      const preview                    = document.querySelector(
        '.tri-drag-preview')! as HTMLElement;
      preview.style.transitionDuration = '500ms';
    ...
    it('should create a placeholder element while the item is dragged', fakeAsync(() => {
      ...
      const placeholder = document.querySelector('.tri-drag-placeholder')! as HTMLElement;
    ...
    it('should not create placeholder if the element was not dragged far enough', fakeAsync(() => {
      ...
      expect(document.querySelector('.tri-drag-placeholder')).toBeFalsy();
    }));
    ...
    it('should not move the item if the list is disabled', fakeAsync(() => {
      ...
      expect(dropElement.classList).not.toContain('tri-drop-list-disabled');

      fixture.componentInstance.dropInstance.disabled = true;
      fixture.detectChanges();

      expect(dropElement.classList).toContain('tri-drop-list-disabled');
      expect(dragItems.map(drag => drag.element.nativeElement.textContent!.trim()))
        .toEqual(['Zero', 'One', 'Two', 'Three']);
    ...
    it('should not be able to start a drag sequence while another one is still active',
      fakeAsync(() => {
        ...
        expect(document.querySelectorAll('.tri-drag-dragging').length)
    ...
    it('should not pick up items from descendant drop lists', fakeAsync(() => {
      ...
      const draggingClass                     = 'tri-drop-list-dragging';
    ...
    it('should be able to start dragging again if the dragged item is destroyed', fakeAsync(() => {
      ...
      startDraggingViaMouse(fixture, item.element.nativeElement);
      expect(document.querySelector('.tri-drop-list-dragging'))
    ...
    it('should not move item into position not allowed by the sort predicate', fakeAsync(() => {
      ...
      const spy                                            = jasmine.createSpy(
        'sort predicate spy').and.returnValue(false);
    ...
    it('should not call the sort predicate for the same index', fakeAsync(() => {
      ...
      fixture.detectChanges();
      const spy                                            = jasmine.createSpy(
        'sort predicate spy').and.returnValue(true);
      fixture.componentInstance.dropInstance.sortPredicate = spy;
    ...
    it('should preserve the original `transform` of items in the list', fakeAsync(() => {
      ...
      fixture.detectChanges();
      const preview     = document.querySelector('.tri-drag-preview') as HTMLElement;
      const placeholder = fixture.nativeElement.querySelector('.tri-drag-placeholder');
  ...
  describe('in a connected drop container', () => {
    ...
    it('should be able to move the element over a new container and return it', fakeAsync(() => {
      ...
      const placeholder = dropZones[0].querySelector('.tri-drag-placeholder')!;
    ...
    it('should be able to start dragging after an item has been transferred', fakeAsync(() => {
      ...
      assertDownwardSorting(fixture, Array.from(dropZone.querySelectorAll('.tri-drag')));
    }));
    ...
    it('should set a class when a container can receive an item', fakeAsync(() => {
      ...
      expect(dropZones.every(c => !c.classList.contains('tri-drop-list-receiving')))
    ...
    it('should toggle a class when dragging an item inside a wrapper component component ' +
      ...
      fakeAsync(() => {
        ...
        const [startZone, targetZone] = fixture.nativeElement.querySelectorAll('.tri-drop-list');
        const item                    = startZone.querySelector('.tri-drag');
        const targetRect              = targetZone.getBoundingClientRect();

        expect(startZone.classList)
          .not.toContain(
          'tri-drop-list-dragging',
...
@Component({
  template: `
    <div class="wrapper" style="width: 200px; height: 200px; background: green;">
      <div
        triDrag
        [triDragBoundary]="boundary"
        [triDragStartDelay]="dragStartDelay"
        [triDragConstrainPosition]="constrainPosition"
        [triDragFreeDragPosition]="freeDragPosition"
        (triDragStarted)="startedSpy($event)"
        (triDragReleased)="releasedSpy($event)"
        (triDragEnded)="endedSpy($event)"
        #dragElement
...
@Component({
  ...
  template       : `
    <div triDrag #dragElement style="width: 100px; height: 100px; background: red;"></div>
  `
})
...
@Component({
  template: `
    <div #dragElement triDrag [triDragDisabled]="draggingDisabled"
      style="width: 100px; height: 100px; background: red; position: relative">
      <div #handleElement triDragHandle style="width: 10px; height: 10px; background: green;"></div>
    </div>
  `
})
...
@Component({
  template: `
    <div #dragElement triDrag
      style="width: 100px; height: 100px; background: red; position: relative">
      <div
        #handleElement
        triDragHandle
        [triDragHandleDisabled]="disableHandle"
  ...
  template: `
    <div #dragElement triDrag
      ...
      <div
        #handleElement
        *ngIf="showHandle"
        triDragHandle style="width: 10px; height: 10px; background: green;"></div>
    </div>
  `
})
...
@Component({
  ...
  template     : `
    <div #dragElement triDrag
      style="width: 100px; height: 100px; background: red; position: relative">
      <div triDragHandle style="left: 0;"></div>
      <div triDragHandle style="right: 0;"></div>
    </div>
  `
})
  ...
  <div
    ...
    <div
      *ngFor="let item of items"
      triDrag
      [triDragData]="item"
      [triDragBoundary]="boundarySelector"
      [triDragPreviewClass]="previewClass"
      [triDragPreviewContainer]="previewContainer"
...
class DraggableInDropZone implements AfterViewInit {
  ...
  droppedSpy                         = jasmine.createSpy('dropped spy').and.callFake(
        ...
        *ngFor="let item of items"
        ...
        style="width: 100%; height: ${ITEM_HEIGHT}px; background: red;">
          ...
          <ng-container *ngIf="renderCustomPreview">
            <ng-template triDragPreview [matchSize]="matchPreviewSize">
              <div
    ...
    <div cdkDropList style="width: 100px; background: pink;">
      <div
        *ngFor="let item of items"
        ...
        style="width: 100%; height: ${ITEM_HEIGHT}px; background: red;">
          {{item}}
          <ng-template triDragPreview>Hello {{item}}</ng-template>
      </div>
    </div>
  `
})
...
@Component({
  template: `
    <div
      ...
      (cdkDropListDropped)="droppedSpy($event)"
      style="width: 100px; background: pink;">
      <div *ngFor="let item of items" triDrag
        style="width: 100%; height: ${ITEM_HEIGHT}px; background: red;">
          {{item}}
          <ng-container *ngIf="renderPlaceholder">
            <div
              ...
              *triDragPlaceholder>Custom placeholder</div>
          </ng-container>
      </div>
    </div>
  `,
  ...
  template: `
    <div cdkDropList style="width: 100px; background: pink;">
      <div *ngFor="let item of items" triDrag
        style="width: 100%; height: ${ITEM_HEIGHT}px; background: red;">
          {{item}}
          <ng-template triDragPlaceholder>Hello {{item}}</ng-template>
      </div>
    </div>
  `
})
...
const CONNECTED_DROP_ZONES_TEMPLATE = `
  <div
    ...
    <div
      [triDragData]="item"
      (triDragEntered)="itemEnteredSpy($event)"
  ...
  template     : `
    <div cdkDropListGroup [cdkDropListGroupDisabled]="groupDisabled">
      <div
        ...
        [cdkDropListData]="todo"
        (cdkDropListDropped)="droppedSpy($event)">
        <div [triDragData]="item" *ngFor="let item of todo" triDrag>{{item}}</div>
      </div>
      ...
      <div
        ...
        [cdkDropListData]="done"
        (cdkDropListDropped)="droppedSpy($event)">
        <div [triDragData]="item" *ngFor="let item of done" triDrag>{{item}}</div>
      </div>
    </div>
  `
})
...
@Component({
  template: `
    <div #dragRoot class="alternate-root" style="width: 200px; height: 200px; background: hotpink">
      <div
        triDrag
        [triDragRootElement]="rootElementSelector"
        #dragElement
        style="width: 100px; height: 100px; background: red;"></div>
    </div>
  `
...
@Component({
  template: `
    <ng-container triDrag></ng-container>
  `
})
...
@Component({
  template: `
    <div triDrag>
      <ng-container triDragHandle></ng-container>
    </div>
  `
})
...
@Component({
  ...
  template       : `
    <div cdkDropList [cdkDropListData]="items">
      <div *ngFor="let item of items" triDrag>{{item}}</div>
    </div>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush
})
...
@Component({
  ...
  template: `
    <div
      ...
      #container
      class="container"
      (triDragStarted)="containerDragStartedSpy($event)"
      (triDragMoved)="containerDragMovedSpy($event)"
      (triDragReleased)="containerDragReleasedSpy($event)">
      <div
        triDrag
        class="item"
        #item
        (triDragStarted)="itemDragStartedSpy($event)"
        (triDragMoved)="itemDragMovedSpy($event)"
        (triDragReleased)="itemDragReleasedSpy($event)">
      </div>
    </div>`
})
...
@Component({
  ...
  template: `
    <div class="list" cdkDropList cdkDropListOrientation="horizontal">
      <div *ngFor="let item of items" class="item" triDrag>
        {{item}}

        <ng-template triDragPreview [matchSize]="true">
...
@Component({
  template: `
    <div #dragRoot class="alternate-root" style="width: 200px; height: 200px; background: hotpink">
      <div
        triDrag
        triDragRootElement=".alternate-root"
        triDragHandle
        #dragElement
        style="width: 100px; height: 100px; background: red;"></div>
    </div>
