...
import { GridsterConfigService } from './gridster-config.constant';
import { GridsterConfig, GridType } from './gridster-config.interface';
...
export class GridsterComponent implements OnInit, OnChanges, OnDestroy, GridsterComponentInterface {
  ...
  updateGrid(): void {
    ...
    this.setGridDimensions();
    this.gridColumns.length = GridsterComponent.getNewArrayLength(this.columns, this.curWidth,
      this.curColWidth);
    this.gridRows.length    = GridsterComponent.getNewArrayLength(this.rows, this.curHeight,
      this.curRowHeight);
  ...
  getNextPossiblePosition(newItem: GridsterItem,
  ...
  getLastPossiblePosition(item: GridsterItem): GridsterItem {
    ...
    farthestItem                               = this.grid.reduce(
      (prev: { y: number, x: number }, curr: GridsterItemComponentInterface) => {
        const currCoords = {
  ...
  findItemWithItemForSwapping(item: GridsterItem): GridsterItemComponentInterface | boolean {
    ...
    for (; widgetsIndex > -1; widgetsIndex--) {
      widget = this.grid[widgetsIndex];
      if (widget.$item !== item && GridsterComponent.checkCollisionTwoItemsForSwaping(widget.$item,
        item)) {
        return widget;
      }
    }
