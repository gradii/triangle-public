...
import {
  Directive, EventEmitter, Inject, Injectable, InjectionToken, Input, OnDestroy, Optional, Output,
  ViewContainerRef,
} from '@angular/core';
...
export class ContextMenuTracker {
  ...
  private static _openContextMenuTrigger?: TriContextMenuTrigger;

  /**
   * Close the previous open context menu and set the given one as being open.
   * @param trigger the trigger for the currently open Context Menu.
   */
  update(trigger: TriContextMenuTrigger) {
...
export const TRI_CONTEXT_MENU_DEFAULT_OPTIONS = new InjectionToken<ContextMenuOptions>(
  'tri-context-menu-default-options',
);
...
@Directive({
  ...
  providers: [
    ...
    {provide: TRI_CONTEXT_MENU_DEFAULT_OPTIONS, useValue: {offsetX: 2, offsetY: 2}},
  ],
})
export class TriContextMenuTrigger implements OnDestroy {
  static ngAcceptInputType_disabled: BooleanInput;
  /** Emits when the attached menu is requested to open. */
  @Output('triContextMenuOpened') readonly opened: EventEmitter<void> = new EventEmitter();
  /** Emits when the attached menu is requested to close. */
  @Output('triContextMenuClosed') readonly closed: EventEmitter<void> = new EventEmitter();
  ...
  constructor(
    ...
    @Inject(TRI_CONTEXT_MENU_DEFAULT_OPTIONS) private readonly _options: ContextMenuOptions,
  ...
  get menuPanel(): TriMenuPanel {
    return this._menuPanel;
  }

  set menuPanel(panel: TriMenuPanel) {
  ...
  private _subscribeToOutsideClicks() {
    if (this._overlayRef) {
      this._overlayRef
        ...
        .subscribe(event => {
          // fix auxclick bug when contextmenu first mouseup
