...
import {
  ConnectedPosition, FlexibleConnectedPositionStrategy, Overlay, OverlayConfig, OverlayRef,
} from '@angular/cdk/overlay';
import { TemplatePortal } from '@angular/cdk/portal';
import {
  Directive, ElementRef, EventEmitter, Inject, Input, NgZone, OnDestroy, Optional, Output,
  ViewContainerRef,
} from '@angular/core';
...
export function isClickInsideMenuOverlay(target: Element): boolean {
  while (target?.parentElement) {
    const isOpenTrigger =
            ...
            target.classList.contains('tri-menu-trigger');
    const isOverlayMenu =
            target.classList.contains('tri-menu') && !target.classList.contains('tri-menu-inline');
...
/**
 * A directive to be combined with TriMenuItem which opens the Menu it is bound to. If the
 ...
 *
 * The directive must be placed along with the `triMenuItem` directive in order to enable full
 * functionality.
 */
@Directive({
  selector: '[triMenuTriggerFor]',
  exportAs: 'triMenuTriggerFor',
  host    : {
    '(keydown)'           : '_toggleOnKeydown($event)',
    '(click)'             : 'toggle()',
    'class'               : 'tri-menu-trigger',
    'aria-haspopup'       : 'menu',
    '[attr.aria-expanded]': 'isMenuOpen()',
  },
})
export class TriMenuItemTrigger implements OnDestroy {
  /** Emits when the attached menu is requested to open */
  @Output('triMenuOpened') readonly opened: EventEmitter<void> = new EventEmitter();
  /** Emits when the attached menu is requested to close */
  @Output('triMenuClosed') readonly closed: EventEmitter<void> = new EventEmitter();
  ...
  private readonly _stopOutsideClicksListener                  = merge(this.closed,
    this._destroyed);
  ...
  private _menuPanel?: TriMenuPanel;

  /** Template reference variable to the menu this trigger opens */
  @Input('triMenuTriggerFor')
  get menuPanel(): TriMenuPanel | undefined {
    return this._menuPanel;
  }

  set menuPanel(panel: TriMenuPanel | undefined) {
  ...
  private _resetPanelMenuStack() {
    // If a TriMenuTrigger is placed in a submenu, each time the trigger is rendered (its parent
    // menu is opened) the panel setter for TriMenuPanel is called. From the first render onward,
    // the attached TriMenuPanel has the MenuStack set. Since we throw an error if a panel already
