...
export class SplitterService {
  ...
  // 按下的时候计算pane的size信息
  dragState(splitbarIndex: number) {
  ...
  // 设置pane大小
  setSize(state: State, distance: number) {
  ...
  clamp(minSize: number, maxSize: number, initialSize: number) {
    return Math.min(maxSize, Math.max(minSize, initialSize));
  }

  // resize pane的大小
  resize(paneState: StateItem, moveSize: number) {
    const pane         = this.getPane(paneState.index);
    const splitterSize = this.containerSize();
    const newSize      = this.clamp(paneState.minSize, paneState.maxSize,
  ...
  isResizable(splitBarIndex: number) {
  ...
  isStaticBar(splitBarIndex: number) {
    const prevPane = this.getPane(splitBarIndex);
    const nextPane = this.getPane(splitBarIndex + 1);
  ...
  // 切换pane展开，收起
  togglePane(paneIndex: number, nearPaneIndex: number, lockStatus?: boolean) {
