...
import { TRI_TAB_GROUP, TriTab } from './tab';
import { TRI_TABS_CONFIG, TriTabsConfig } from './tab-config';
...
const _TriTabGroupMixinBase: CanColorCtor & CanDisableRippleCtor & typeof TriTabGroupMixinBase =
        mixinColor(mixinDisableRipple(TriTabGroupMixinBase), 'primary');

interface TriTabGroupBaseHeader {
  _alignInkBarToSelectedTab: () => void;
  focusIndex: number;
}

/**
 * Base class with all of the `TriTabGroupBase` functionality.
 * @docs-private
 */
@Directive()
// tslint:disable-next-line:class-name
export abstract class _TriTabGroupBase extends _TriTabGroupMixinBase implements AfterContentInit,
   ...
   */
  abstract _allTabs: QueryList<TriTab>;
  ...
  set backgroundColor(value: ThemePalette) {
    ...
    nativeElement.classList.remove(`tri-background-${this.backgroundColor}`);

    if (value) {
      nativeElement.classList.add(`tri-background-${value}`);
    }
  ...
  ngAfterContentChecked() {
    ...
    this._tabs.forEach((tab: TriTab, index: number) => {
  ...
  _getTabLabelId(i: number): string {
    return `tri-tab-label-${this._groupId}-${i}`;
  }

  /** Returns a unique id for each tab content element */
  _getTabContentId(i: number): string {
    return `tri-tab-content-${this._groupId}-${i}`;
  }
  ...
  _handleClick(tab: TriTab, tabHeader: TriTabGroupBaseHeader, index: number) {
    if (!tab.disabled) {
  ...
  _getTabIndex(tab: TriTab, idx: number): number | null {
    if (tab.disabled) {
      return null;
    }
  ...
  private _subscribeToAllTabChanges() {
    ...
    this._allTabs.changes
      ...
      .subscribe((tabs: QueryList<TriTab>) => {
        this._tabs.reset(tabs.filter(tab => {
          // @breaking-change 10.0.0 Remove null check for `_closestTabGroup`
          // once it becomes a required parameter in TriTab.
  ...
  private _createChangeEvent(index: number): TriTabChangeEvent {
   ...
   * on the TriTab component, whereas the data binding is inside the TriTabGroup. In order for the
...
export class TriTabGroup extends _TriTabGroupBase {
