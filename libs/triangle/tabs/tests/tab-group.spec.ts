...
describe('TriTabGroup', () => {
  ...
  describe('nested tabs', () => {
    ...
    it('should pick up indirect descendant tabs', fakeAsync(() => {
      ...
      expect(tabs.map((tab: TriTab) => tab.textLabel)).toEqual(['One', 'Two']);
    }));
  });
...
class SimpleTabsTestApp {
  @ViewChildren(TriTab) tabs: QueryList<TriTab>;
...
class NestedTabs {
  @ViewChildren(TriTabGroup) groups: QueryList<TriTabGroup>;
}
