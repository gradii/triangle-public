...
import {
  Component, ContentChild, EventEmitter, Input, OnChanges, OnDestroy, OnInit, Output, SimpleChanges, TemplateRef, ViewChild
} from '@angular/core';
...
export class TransferComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() transferToTarget = new EventEmitter<null | any>();
  @Output() transferToSource = new EventEmitter<null | any>();
  ...
  setI18nText() {
    this.i18nCommonText   = this.i18n.translate('common');
    this.i18nSubscription = this.i18n.localeChange.subscribe((data) => {
      this.i18nCommonText = this.i18n.translate('common');
    });
  }
  ...
  checkboxChange(direction: TransferDirection, event: any) {
    if (direction === TransferDirection.SOURCE) {
      this.sourceDisplayOption.filter(
        item => item.checked).length > 0 ? this.targetCanTransfer = true : this.targetCanTransfer = false;
      this.listTotalCheck(direction);
    } else if (direction === TransferDirection.TARGET) {
      this.targetDisplayOption.filter(
        item => item.checked).length > 0 ? this.sourceCanTransfer = true : this.sourceCanTransfer = false;
      this.listTotalCheck(direction);
    }
  }
  ...
  transferHandle(direction: TransferDirection, changeData?: object) {
    if (direction === TransferDirection.TARGET) {
      ...
      if (this.sourceCustomViewTemplate) {
        this.transferToTarget.next(null);
      } else {
      ...
      if (this.targetCustomViewTemplate) {
        this.transferToSource.next(null);
      } else {
        changeData === undefined
